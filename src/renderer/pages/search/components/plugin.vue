        ...
        <a-tabs default-active-key="1">
          <a-tab-pane key="1" tab="功能关键字">
            <div class="feature-container">
              <div class="desc-item" v-for="item in pluginDetail.features" v-bind:key="item">
                <div>{{item.explain}}</div>
                <a-tag @click="openPlugin({cmd, plugin: pluginDetail, feature: item, router: $router})" v-for="cmd in item.cmds" v-bind:key="cmd">{{cmd}}</a-tag>
              </div>
            </div>
